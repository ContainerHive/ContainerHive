...
func TestTarFilePath(t *testing.T) {
	tests := []struct {
		...
		{"dist", "myimg", "1.0", filepath.Join("dist", "myimg", "1.0", "image.tar")},
		{"/abs/dist", "app", "latest", filepath.Join("/abs/dist", "app", "latest", "image.tar")},
		{"dist", "img", "1.0-slim", filepath.Join("dist", "img", "1.0-slim", "image.tar")},
	}

	for _, tt := range tests {
		t.Run(tt.name+":"+tt.tag, func(t *testing.T) {
			got := TarFilePath(tt.distPath, tt.name, tt.tag)
			if got != tt.want {
				t.Errorf("TarFilePath(%q, %q, %q) = %q, want %q", tt.distPath, tt.name, tt.tag, got, tt.want)
			}
		})
	}
}
